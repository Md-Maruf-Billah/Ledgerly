...
async def list_tasks(user: dict = Depends(get_current_user)):
    tasks = await task_service.get_tasks(user["user_id"], user["token"])
...
async def create_task(payload: TaskCreate, user: dict = Depends(get_current_user)):
    try:
        task = await task_service.create_task(user["user_id"], payload.model_dump(), user["token"])
...
async def mark_done(task_id: str, user: dict = Depends(get_current_user)):
    try:
        task = await task_service.mark_task_done(user["user_id"], task_id, user["token"])
        notif = await notification_service.create_completion_notification(
            user["user_id"], task_id, task["name"], user["token"]
        )
...
async def delete_task(task_id: str, user: dict = Depends(get_current_user)):
    await task_service.delete_task(user["user_id"], task_id, user["token"])
